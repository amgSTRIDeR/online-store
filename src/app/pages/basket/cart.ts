...
export class CartPage {
    ...
    if (cartSection) {
      if (cart.cartArray.length) {
        ...
        for (let i = 0; i < cart.cartArray.length; i++) {
          ...
          headInfoTime.textContent = `Время игры: ${GamesCollection[
            cardIndex
          ].GameTime.join('-')} минут`;
        ...
        wrapperDiv.innerHTML = 'Корзина пуста';
      }
    }
  ...
  static basketCardsRenew() {
    ...
    if (basketCards instanceof HTMLElement) {
      ...
      basketCards.style.gridTemplateRows = `repeat(${CartPage.itemsLimit < cart.cartArray.length
          ? CartPage.itemsLimit

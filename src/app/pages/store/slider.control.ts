  ...
  static renewSlider() {
    ...
    if (cardsElement instanceof HTMLDivElement) {
      if (StoreSlider.direction === 0) {
        ...
        cardsElement.style.transform = `translateX(${(StoreSlider.currentPageNumber - 1) * -72
          }vw)`;
      } else {
        cardsElement.style.transform = `translateX(0vw)`;
        cardsElement.style.transform = `translateY(${(StoreSlider.currentPageNumber - 1) * -30
          }vw)`;
      }
